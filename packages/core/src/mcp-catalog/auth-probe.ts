 ...
 * whatever those return. The cascade is handed `probeFetch` so every request it
 * makes is subject to the same public-destination, no-redirect, bounded-body
 * rules as the ones made here — checking only the URLs this function sees would
 * leave the redirects inside discovery unguarded.
 */
async function resolveAuthServerOrigin(
  ...
  const discovery = await resolveMCPOAuthDiscovery(wwwAuthenticate, remoteUrl, probeFetch);
