...
describe('OpenCodeTool', () => {
  ...
  describe('executeTask Integration', () => {
    it('keeps permission supervision paused until auto-grant succeeds', async () => {
      ...
      const { onActivity, execute } = permissionTool();
      ...
      expect(onActivity.mock.calls).toEqual([
        ...
        [{ type: 'waiting_finished', id: 'permission-1', outcome: 'resolved' }],
        [{ type: 'progress', detail: 'session.idle' }],
      ]);
    });
    ...
    it('keeps a failed grant paused until the configured permission timeout', async () => {
      ...
      try {
        ...
        expect(onActivity.mock.calls).toEqual([
          ...
          [{ type: 'waiting_finished', id: 'permission-1', outcome: 'timed_out' }],
        ]);
      } finally {
        releaseStream?.();
        await execution;
